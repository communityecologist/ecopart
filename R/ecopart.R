...
ecopart.pair	<-	function(d1, d2, index="ruzicka", components="two"){
  ...
  if(components=="two"){
    ...
    names(Res) <- c("Changes in beta by abundance loss (??BetaLoss)",
                    "Changes in beta by abundance gain (??BetaGain)")
  } else if(components=="four"){
    Res        <- lapply(1:4, function(x) matrix(nrow=N, ncol=N))
    names(Res) <- c("Homogenization by abundance loss (??BetaLoss-)",
                    "Heterogenization by abundance loss (??BetaLoss+)",
                    "Homogenization by abundance gain (??BetaGain-)",
                    "Heterogenization by abundance gain (??BetaGain+)")
  } else if(components=="six"){
    Res        <- lapply(1:6, function(x) matrix(nrow=N, ncol=N))
    names(Res) <- c("??Beta1", "??Beta2", "??Beta3", "??Beta4", "??Beta5", "??Beta6")
...
ecopart.multi <-	function(d1, d2, components="two"){
  ...
  if(components=="two"){
    ...
    Res						<-	DBeta
    rownames(Res)	<-	c("??BetaLoss-", "??BetaLoss+", "??BetaGain-", "??BetaGain+")
